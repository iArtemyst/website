...
function MobileLazyCardInternals({dataArray}: {dataArray: IProjectLinkCard}) {
    ...
    return (
        <div className={`relative grid w-fit h-auto p-[8px] transition-all duration-300 rounded-md place-items-center overflow-clip`}>
            <div className="relative content-center w-[120px] h-[90%] self-start overflow-hidden rounded-md z-20 shadow-[0px_0px_12px_#00000090]">
                <MobileProjectVideo src={dataArray.mediaLink} />
            </div>
...
function MobileGroupProjectCards({dataArray}: {dataArray: IProjectLinkCard}) {
    return (
        <div className={`relative w-fit h-fit place-self-center`}>
            <div className={`bg-cardBGInactiveColor active:bg-cardBGColor w-fit h-fit rounded-[8px] opacity-80 active:opacity-100 active:scale-[110%] transition-all duration-200`} >
                <Link href={dataArray.cardLink}>
                    <MobileLazyCardInternals dataArray={dataArray}/>
                    {
                        typeof dataArray.errorText === "string" ?
                            <div>
                                <p className={`${fonts.dotoBlack.className} text-nowrap absolute w-full h-fit text-center top-[-12px] text-textVariant text-[6px]`}>{dataArray.errorText}</p>
                            </div>
                        :
                            <></>
                    }
        ...
        <div className="">
            {
                CheckIfMobileBrowser() === false ?
                    ...
                    <div className={`grid grid-cols-2 w-[80%] h-full absolute left-[50%] -translate-x-[50%] top-[50%] -translate-y-[50%] gap-[12px] place-content-center`}>
                        {
                            dataArray.map((data, i) => <MobileGroupProjectCards key={i} dataArray={data}/> )
                        }
                    </div>
            }
        </div>
