...
function LazyVideoCore({src, autoplay, controls, muted, loop, className, onEnter, onLeave} : { 
    ...
    loop:boolean,
    className: string,
    onEnter: (e: React.MouseEvent<HTMLVideoElement>) => void,
    onLeave: (e: React.MouseEvent<HTMLVideoElement>) => void, 
}) 
{
    ...
    return loadedSrc === '' ?
        <Loading/> :
        <video onMouseEnter={onEnter} onMouseLeave={onLeave} src={loadedSrc} width="100%" height="100%" autoPlay={autoplay} controls={controls} muted={muted} loop={loop} className={className}/>
}
